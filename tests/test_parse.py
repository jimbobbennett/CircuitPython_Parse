...
class UrlParseTestCase(unittest.TestCase):
    def checkRoundtrips(self, url, parsed, split):
        ...
        result2 = parse.urlunparse(result)
        self.assertEqual(result2, url)
        self.assertEqual(result2, result.geturl())

        # the result of geturl() is a fixpoint; we can always parse it
        # again to get the same result:
        result3 = parse.urlparse(result.geturl())
        ...
        result2 = parse.urlunsplit(result)
        self.assertEqual(result2, url)
        self.assertEqual(result2, result.geturl())

        # check the fixpoint property of re-parsing the result of geturl()
        result3 = parse.urlsplit(result.geturl())
        self.assertEqual(result3.geturl(), result.geturl())
        self.assertEqual(result3, result)
        self.assertEqual(result3.scheme, result.scheme)
        self.assertEqual(result3.netloc, result.netloc)
    ...
    def test_qsl(self):
        for orig, expect in parse_qsl_test_cases:
            result = parse.parse_qsl(orig, keep_blank_values=True)
            self.assertEqual(result, expect, "Error parsing %r" % orig)
            expect_without_blanks = [v for v in expect if len(v[1])]
            result = parse.parse_qsl(orig, keep_blank_values=False)
            self.assertEqual(result, expect_without_blanks, "Error parsing %r" % orig)

    def test_qs(self):
        for orig, expect in parse_qs_test_cases:
            result = parse.parse_qs(orig, keep_blank_values=True)
            self.assertEqual(result, expect, "Error parsing %r" % orig)
            expect_without_blanks = {v: expect[v] for v in expect if len(expect[v][0])}
            result = parse.parse_qs(orig, keep_blank_values=False)
            self.assertEqual(result, expect_without_blanks, "Error parsing %r" % orig)
    ...
    def test_http_roundtrips(self):
        # parse.urlsplit treats 'http:' as an optimized special case,
    ...
    def checkJoin(self, base, relurl, expected):
        ...
        self.assertEqual(parse.urljoin(base, relurl), expected)
        bytes_components = baseb, relurlb, expectedb = [x.encode("ascii") for x in str_components]
        self.assertEqual(parse.urljoin(baseb, relurlb), expectedb)
    ...
    def test_unparse_parse(self):
        ...
        for u in str_cases + bytes_cases:
            self.assertEqual(parse.urlunsplit(parse.urlsplit(u)), u)
            self.assertEqual(parse.urlunparse(parse.urlparse(u)), u)
    ...
    def test_RFC2368(self):
        ...
        self.assertEqual(parse.urlparse("mailto:1337@example.org"), ("mailto", "", "1337@example.org", "", "", ""))
    ...
    def test_RFC2732(self):
        ...
        for invalid_url in str_cases + bytes_cases:
            self.assertRaises(ValueError, parse.urlparse, invalid_url)
    ...
    def test_attributes_bad_port(self):
        ...
        for bytes in (False, True):
            for parse_method in (parse.urlsplit, parse.urlparse):
                for port in ("foo", "1.5", "-1", "0x10"):
                    with self.subTest(bytes=bytes, parse=parse_method, port=port):
    ...
    def test_noslash(self):
        ...
        self.assertEqual(parse.urlparse("http://example.com?blahblah=/foo"), ("http", "example.com", "", "", "blahblah=/foo", ""))
        self.assertEqual(parse.urlparse(b"http://example.com?blahblah=/foo"), (b"http", b"example.com", b"", b"", b"blahblah=/foo", b""))
    ...
    def test_withoutscheme(self):
        ...
        self.assertEqual(parse.urlparse("path"), ("", "", "path", "", "", ""))
        self.assertEqual(parse.urlparse("//www.python.org:80"), ("", "www.python.org:80", "", "", "", ""))
        self.assertEqual(parse.urlparse("http://www.python.org:80"), ("http", "www.python.org:80", "", "", "", ""))
        # Repeat for bytes input
        self.assertEqual(parse.urlparse(b"path"), (b"", b"", b"path", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"//www.python.org:80"), (b"", b"www.python.org:80", b"", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"http://www.python.org:80"), (b"http", b"www.python.org:80", b"", b"", b"", b""))

    def test_portseparator(self):
        # Issue 754016 makes changes for port separator ':' from scheme separator
        self.assertEqual(parse.urlparse("http:80"), ("http", "", "80", "", "", ""))
        self.assertEqual(parse.urlparse("https:80"), ("https", "", "80", "", "", ""))
        self.assertEqual(parse.urlparse("path:80"), ("path", "", "80", "", "", ""))
        self.assertEqual(parse.urlparse("http:"), ("http", "", "", "", "", ""))
        self.assertEqual(parse.urlparse("https:"), ("https", "", "", "", "", ""))
        self.assertEqual(parse.urlparse("http://www.python.org:80"), ("http", "www.python.org:80", "", "", "", ""))
        # As usual, need to check bytes input as well
        self.assertEqual(parse.urlparse(b"http:80"), (b"http", b"", b"80", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"https:80"), (b"https", b"", b"80", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"path:80"), (b"path", b"", b"80", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"http:"), (b"http", b"", b"", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"https:"), (b"https", b"", b"", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"http://www.python.org:80"), (b"http", b"www.python.org:80", b"", b"", b"", b""))

    def test_usingsys(self):
        # Issue 3314: sys module is used in the error
        self.assertRaises(TypeError, parse.urlencode, "foo")

    def test_anyscheme(self):
        # Issue 7904: s3://foo.com/stuff has netloc "foo.com".
        self.assertEqual(parse.urlparse("s3://foo.com/stuff"), ("s3", "foo.com", "/stuff", "", "", ""))
        self.assertEqual(parse.urlparse("x-newscheme://foo.com/stuff"), ("x-newscheme", "foo.com", "/stuff", "", "", ""))
        self.assertEqual(
            parse.urlparse("x-newscheme://foo.com/stuff?query#fragment"), ("x-newscheme", "foo.com", "/stuff", "", "query", "fragment")
        )
        self.assertEqual(parse.urlparse("x-newscheme://foo.com/stuff?query"), ("x-newscheme", "foo.com", "/stuff", "", "query", ""))

        # And for bytes...
        self.assertEqual(parse.urlparse(b"s3://foo.com/stuff"), (b"s3", b"foo.com", b"/stuff", b"", b"", b""))
        self.assertEqual(parse.urlparse(b"x-newscheme://foo.com/stuff"), (b"x-newscheme", b"foo.com", b"/stuff", b"", b"", b""))
        self.assertEqual(
            parse.urlparse(b"x-newscheme://foo.com/stuff?query#fragment"),
            (b"x-newscheme", b"foo.com", b"/stuff", b"", b"query", b"fragment"),
        )
        self.assertEqual(parse.urlparse(b"x-newscheme://foo.com/stuff?query"), (b"x-newscheme", b"foo.com", b"/stuff", b"", b"query", b""))

    def test_default_scheme(self):
        # Exercise the scheme parameter of urlparse() and urlsplit()
        for func in (parse.urlparse, parse.urlsplit):
    ...
    def test_parse_fragments(self):
        ...
        for url, attr, expected_frag in tests:
            for func in (parse.urlparse, parse.urlsplit):
                if attr == "params" and func is parse.urlsplit:
                    attr = "path"
    ...
    def test_mixed_types_rejected(self):
        ...
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlparse("www.python.org", b"http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlparse(b"www.python.org", "http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlsplit("www.python.org", b"http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlsplit(b"www.python.org", "http")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlunparse((b"http", "www.python.org", "", "", "", ""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlunparse(("http", b"www.python.org", "", "", "", ""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlunsplit((b"http", "www.python.org", "", "", ""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urlunsplit(("http", b"www.python.org", "", "", ""))
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urljoin("http://python.org", b"http://python.org")
        with self.assertRaisesRegex(TypeError, "Cannot mix str"):
            parse.urljoin(b"http://python.org", "http://python.org")
    ...
    def test_parse_qs_encoding(self):
        result = parse.parse_qs("key=\u0141%E9", encoding="latin-1")
        self.assertEqual(result, {"key": ["\u0141\xE9"]})
        result = parse.parse_qs("key=\u0141%C3%A9", encoding="ascii")
        self.assertEqual(result, {"key": ["\u0141\ufffd\ufffd"]})
        result = parse.parse_qs("key=\u0141%E9-", encoding="ascii")
        self.assertEqual(result, {"key": ["\u0141\ufffd-"]})
        result = parse.parse_qs("key=\u0141%E9-", encoding="ascii", errors="ignore")
        self.assertEqual(result, {"key": ["\u0141-"]})

    def test_parse_qsl_max_num_fields(self):
        with self.assertRaises(ValueError):
            parse.parse_qs("&".join(["a=a"] * 11), max_num_fields=10)
        with self.assertRaises(ValueError):
            parse.parse_qs(";".join(["a=a"] * 11), max_num_fields=10)
        parse.parse_qs("&".join(["a=a"] * 10), max_num_fields=10)
    ...
    def test_urlencode_quote_via(self):
        ...
        result = parse.urlencode({"a": "some value/another"}, quote_via=parse.quote)
        self.assertEqual(result, "a=some%20value%2Fanother")
        result = parse.urlencode({"a": "some value/another"}, safe="/", quote_via=parse.quote)
        self.assertEqual(result, "a=some%20value/another")

    def test_quote_from_bytes(self):
        self.assertRaises(TypeError, parse.quote_from_bytes, "foo")
    ...
    def test_unquote_to_bytes(self):
        result = parse.unquote_to_bytes("abc%20def")
        self.assertEqual(result, b"abc def")
        result = parse.unquote_to_bytes("")
        self.assertEqual(result, b"")

    def test_quote_errors(self):
        self.assertRaises(TypeError, parse.quote, b"foo", encoding="utf-8")
        self.assertRaises(TypeError, parse.quote, b"foo", errors="strict")

    def test_issue14072(self):
        p1 = parse.urlsplit("tel:+31-641044153")
        self.assertEqual(p1.scheme, "tel")
        self.assertEqual(p1.path, "+31-641044153")
        p2 = parse.urlsplit("tel:+31641044153")
        self.assertEqual(p2.scheme, "tel")
        self.assertEqual(p2.path, "+31641044153")
        # assert the behavior for urlparse
        p1 = parse.urlparse("tel:+31-641044153")
        self.assertEqual(p1.scheme, "tel")
        self.assertEqual(p1.path, "+31-641044153")
        p2 = parse.urlparse("tel:+31641044153")
        self.assertEqual(p2.scheme, "tel")
        self.assertEqual(p2.path, "+31641044153")

    def test_port_casting_failure_message(self):
        message = "Port could not be cast to integer value as oracle"
        p1 = parse.urlparse("http://Server=sde; Service=sde:oracle")
        with self.assertRaisesRegex(ValueError, message):
            p1.port

        p2 = parse.urlsplit("http://Server=sde; Service=sde:oracle")
        with self.assertRaisesRegex(ValueError, message):
            p2.port

    def test_telurl_params(self):
        p1 = parse.urlparse("tel:123-4;phone-context=+1-650-516")
        self.assertEqual(p1.scheme, "tel")
        self.assertEqual(p1.path, "123-4")
        self.assertEqual(p1.params, "phone-context=+1-650-516")

        p1 = parse.urlparse("tel:+1-201-555-0123")
        self.assertEqual(p1.scheme, "tel")
        self.assertEqual(p1.path, "+1-201-555-0123")
        self.assertEqual(p1.params, "")

        p1 = parse.urlparse("tel:7042;phone-context=example.com")
        self.assertEqual(p1.scheme, "tel")
        self.assertEqual(p1.path, "7042")
        self.assertEqual(p1.params, "phone-context=example.com")

        p1 = parse.urlparse("tel:863-1234;phone-context=+1-914-555")
        self.assertEqual(p1.scheme, "tel")
